/*
    iterator 必须有5个 associated types，用于algorithm使用：value_type/difference/category/ptr/ref
    如果iterator是class，直接iterator::value_type就好，但如果iterator是指针形式，为了
    拿到其types，设计出iterator_traits作为中间层，对 class-iterator和nonclass-iterator 统一
    包装如下，
    ...
    包装nonclass-iterator:
      template <class T>
      struct iterator_traits<class T*> {
        typedef T value_type;
      }

    经过iterator_traits的包装，algorithm便可以如下方式进行使用：
    void algo(...) {
...
//针对 nonclass-iterator(raw pointer) 设计的偏特化版本
template<class T>
