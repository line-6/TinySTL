...
inline ForwardIterator _uninitialized_copy_aux(InputIterator first,
                                               ...
                                               ForwardIterator result,
                                               true_type) {
  return TinySTL::copy(first, last, result);// in stl_algobase.h
}
...
inline ForwardIterator uninitialized_copy_copy(InputIterator1 first1,
                                               ...
                                               ForwardIterator result) {
  ForwardIterator mid = TinySTL::uninitialized_copy(first1, last1, result);
  try {
    return TinySTL::uninitialized_copy(first2, last2, mid);
  } catch (std::exception &) {
...
inline ForwardIterator uninitialized_fill_copy(ForwardIterator result,
                                               ...
                                               InputIterator first,
                                               InputIterator last) {
  TinySTL::uninitialized_fill(result, mid, val);
  try {
    return TinySTL::uninitialized_copy(first, last, mid);
  } catch (std::exception &) {
    TinySTL::destroy(result, mid);
    throw;
  }
...
inline void uninitialized_copy_fill(InputIterator first1, InputIterator last1,
  ...
  ForwardIterator mid2 = TinySTL::uninitialized_copy(first1, last1, first2);
  try {
    TinySTL::uninitialized_fill(mid2, last2, val);
  } catch (std::exception &) {
    TinySTL::destroy(first2, mid2);
    throw;
  }
}
